...
export class DiscordAlerter {
  ...
  async evaluateAndAlert(llmProvider, delta, memory) {
    ...
    // Jev typed triage first: the decision space (shouldAlert/tier) is declared
    // up front, so there is no JSON to parse and no field to malform. Advisory —
    // triageSignals() returns null on any problem, so the paths below still run.
    let evaluation = null;
    ...
    try {
      ...
      if (jevEval) {
        ...
        console.log(`[Discord] Jev triage: shouldAlert=${jevEval.shouldAlert} tier=${jevEval.tier} (${jevEval._jev?.latency}s)`);
      }
    } catch (err) {
      console.warn('[Discord] Jev triage failed, falling back to LLM/rules:', err.message);
    }
