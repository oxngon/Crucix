...
import { triageSignals, mergeDecisionProse } from './jev-triage.mjs';
...
export class TelegramAlerter {
  ...
  async evaluateAndAlert(llmProvider, delta, memory) {
    ...
    // 2. Jev typed triage first (structured decision, no JSON to parse).
    //    Advisory: triageSignals() returns null on any problem, so the LLM and
    //    rule-based paths below still run as before.
    let evaluation = null;

    try {
      const jevEval = await triageSignals(newSignals, delta);
      if (jevEval) {
        evaluation = mergeDecisionProse(jevEval, newSignals, delta, this);
        console.log(`[Telegram] Jev triage: shouldAlert=${jevEval.shouldAlert} tier=${jevEval.tier} (${jevEval._jev?.latency}s)`);
      }
    } catch (err) {
      console.warn('[Telegram] Jev triage failed, falling back to LLM/rules:', err.message);
    }

    // Try LLM evaluation next, fall back to rule-based if unavailable
    if (!evaluation && llmProvider?.isConfigured) {
      try {
