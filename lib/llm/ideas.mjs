...
function compactSweepForLLM(data, delta, previousIdeas) {
  ...
  // Economic indicators — each tagged with its series date so the model can tell a
  // days-old official print from a live quote. FRED lags; Yahoo does not.
  if (data.fred?.length) {
    const key = data.fred.filter(f => ['VIXCLS', 'DFF', 'DGS10', 'DGS2', 'T10Y2Y', 'BAMLH0A0HYM2', 'DTWEXBGS', 'MORTGAGE30US'].includes(f.id));
    const liveVix = data.markets?.vix;
    const liveVixNote = liveVix?.value != null
      ? ` | VIX_LIVE=${liveVix.value}${liveVix.changePct != null ? ` (${liveVix.changePct >= 0 ? '+' : ''}${liveVix.changePct}%)` : ''} [prefer this over VIXCLS; the FRED series lags]`
      : '';
    sections.push(`ECONOMIC: ${key.map(f => `${f.id}=${f.value}${f.date ? `@${f.date}` : ''}${f.momChange ? ` (${f.momChange > 0 ? '+' : ''}${f.momChange})` : ''}`).join(', ')}${liveVixNote}`);
  }

  // Energy — annotate any leg that is carried-forward or lagging, so the model does not
  // build a dislocation thesis on a vintage mismatch (see the Brent/WTI case).
  if (data.energy) {
    const v = data.energy.vintage || {};
    const tag = (k) => {
      const s = v[k]?.source;
      if (s === 'yahoo-carried') return ` [CARRIED from ${(v[k].asOf||'').slice(0,16)}Z — Yahoo failed this sweep; last known-good live price]`;
      if (s === 'eia-lagged') return ` [LAGGING official print${v[k].asOf?` ${v[k].asOf}`:''} — not live]`;
      return '';
    };
    sections.push(`ENERGY: WTI=$${data.energy.wti}${tag('wti')}, Brent=$${data.energy.brent}${tag('brent')}, NatGas=$${data.energy.natgas}${tag('natgas')}, CrudeStocks=${data.energy.crudeStocks}bbl`);
  }
