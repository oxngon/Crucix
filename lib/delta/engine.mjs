...
const NUMERIC_METRICS = [
  // VIX must use the same source as WTI/Brent/gold below it (live Yahoo), otherwise the
  // delta engine compares a live oil price against a days-old VIX print.
  { key: 'vix', extract: d => d.markets?.vix?.value ?? d.fred?.find(f => f.id === 'VIXCLS')?.value, label: 'VIX' },
